...
- **`bin/coverage-report.py`** — Reads each module's Kover XML (`<module>/build/reports/kover/reportDebug.xml`) and prints a per-module coverage table as Markdown (`--format md`, for the Actions run summary) or the JSON `reports` manifest (`--format reports`, consumed by the `jmerhar/coverage` site's `make-meta.py`); with `--gate` it instead **enforces the per-module line-coverage gate** (thresholds in `GATES`) and exits non-zero on a regression — this is the CI gate (chosen over `koverVerifyDebug`, which mis-applies wildcard excludes). Used by `test.yml`.
