...
- **`bin/coverage-report.py`** — Reads each module's Kover XML (`<module>/build/reports/kover/reportDebug.xml`) and prints a per-module coverage table as Markdown (`--format md`, for the Actions run summary) or a standalone HTML landing page (`--format html`, for the Pages site). Used by the `test.yml` workflow.
- **`bin/build-coverage-site.sh`** — Assembles the three modules' Kover `htmlDebug` reports plus a generated `index.html` (via `bin/coverage-report.py`) into `coverage-site/` (or `$1`) for publishing to the `jmerhar/coverage` Pages repo. Used by `test.yml`.
