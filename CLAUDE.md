...
- **`FallbackPriceFetcher`** — `PriceFetcher` that tries a list of fetchers in order and returns the first result that **actually has prices**: a successful-but-empty response (HTTP 200 with no day-ahead data) falls through to the next source. If no fetcher yields prices it returns an empty result (treated as "not enough data") when at least one responded; the last exception is thrown only when every fetcher failed outright. Used for all multi-source zones (e.g. NL: ENTSO-E → Energy-Charts → EnergyZero). Carries a **chain-level time budget** (`budgetMillis`, default `DEFAULT_BUDGET_MILLIS` = 30s, with an injectable `nanoTime` for tests): because the fetchers run in sequence their per-request timeouts add up, so once the budget is spent the remaining sources are not started. Checked *between* fetchers, not enforced on them — an in-flight request is not interrupted, so the total is bounded at roughly the budget plus one source's timeout rather than one timeout per source in the zone. The first source is always attempted regardless (the budget caps compounding waits, not fetching at all), which is also what guarantees there is an outcome to return or throw. A fast refusal (an ENTSO-E 503 lands in under ~4s) never trips it, so the budget only bites on a hanging source.
