...
- **`bin/data/build-ev-db.py`** — Builds the bundled EV vehicle database (`app/src/main/assets/ev-vehicles.json`) by merging two open datasets via per-source adapters into a normalised schema. Cars only; source #2 wins on collision (newer data). Called by `make ev-db`. Both the script and the generated asset are committed.
...
```bash
./gradlew test                   # Run all unit tests (726 tests)
