...
- **`reference/`** — Research and reference: multi-zone API comparison, Play Store publishing guide, country & language coverage audit, Help & feedback system (`help-support-system.md` — the shipped in-app support + Worker + email architecture)
...
```bash
./gradlew test                   # Run all unit tests (713 tests)
...
- **Excluded classes must contain no logic** — only presentation or thin framework glue. What's excluded (per module `kover { reports { filters { excludes } } }`):
  ...
  - Thin wrappers over untestable SDKs / IO — `PlayBillingRepository` (Play Billing), `WearableSync`, `WearableStatsBridge`, and `WearableUsageBridge` (Wearable Data Layer), `HttpStatsPoster` (HTTP). Each SDK/IO boundary is isolated behind an interface (`BillingRepository`, `WearSync`, `WatchStatsBridge`, `WatchUsageBridge`, `StatsPoster`) with a **fake used in ViewModel/reporter tests**, so the decision logic stays in the covered class.
...
- **`model/ApplianceSort`** — `@Serializable` ordering spec: `SortKey` (`CUSTOM`/`FREQUENCY`/`RECENCY`/`NAME`/`DURATION`/`TYPE`), `SortCriterion` (key + `descending`), `ApplianceSort` (primary + tie-breakers; `isCustom`). Also `ApplianceUsage` (all-time `count` + `lastUsedMs`) and the `EvPosition` enum (`INTERLEAVED`/`FIRST`/`LAST`, persisted by `key`, home-screen placement).
...
- **`util/ApplianceSorting`** — Pure appliance ordering: `sortAppliances(list, sort, usage)` (stable comparator chain, EV Duration = +∞, insertion order as final tie-break, custom = stored order), `hasCollisions`/`nextAssignableKeys` (drive collision-gated tie-breaker disclosure), `combineUsage` (phone + watch), `mergeForHome(...) : HomeChipLayout` (`Flat`/`Sectioned` — folds vehicles in by `EvPosition` + separate-section, EVs name-ordered), and the sort-control edit helpers (`withPrimary`/`withLevelKey`/`withToggledDirection`/`withAddedTiebreaker`/`withoutLevel`). Consumed by `SweetSpotViewModel` and the Settings sort control.
