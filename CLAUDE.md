...
```bash
./gradlew test                   # Run all unit tests (734 tests)
...
- **`util/CheapestWindowFinder`** — Pure functions implementing the sliding window algorithm. Works with any slot duration (15min, 30min, 60min). Converts requested duration to "slot units" and multiplies by `slotMinutes / 60.0` for EUR costs. Supports fractional slots. Split into `findBestStartIndex`, `computeWindowCost`, `buildBreakdown`, and `buildWindowAt`. `findCheapestWindow` returns the single cheapest window; `findWindowAlternatives` returns the "earlier path" — the cheapest window first, then each successively-cheapest window that starts earlier, down to the earliest (start clamped to now). Cost increases monotonically along the list, so the results screen can step "earlier" (costlier, sooner) and "cheaper" (back toward the cheapest) by moving an offset. Both functions accept an optional `notLaterThan` deadline that caps candidate start indices so the window's clamped end time does not exceed it (`null`, the default, imposes no constraint) — a pure capability retained for tests/reuse. The phone no longer uses this cap: it applies its "ready by" deadline as a **soft default** at the ViewModel level (passes `null` here, builds the full earlier-path, and sets the default offset to the cheapest window meeting the deadline via `deadlineDefaultOffset`), so "Cheaper" can still browse cheaper windows that finish after it.
