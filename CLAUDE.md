...
- **`coverage.toml`** (repo root) — Declares each module as a coverage suite: its label, its Kover XML report, its `htmlDebug` directory, and its line-coverage gate. The tooling that reads it — the run-summary table, the gate, and the publishing to the `jmerhar/coverage` site — is **shared across every project on that site** and lives there, invoked from `test.yml` as three composite actions (`summary`, `gate`, `publish`, pinned to `@v1`). There are no coverage scripts in this repo. Change a threshold or add a module here; anything else belongs in `jmerhar/coverage`.
...
Shared bash helpers live in **`bin/lib/`** — `log.sh` (consistent `log_info`/`log_success`/`log_warn`/`log_error`/`die`), `locale.sh` (language ↔ Play-locale maps), `gallery.sh`, and `require.sh`, aggregated by `common.sh`; all scripts route status/warnings/errors through `log.sh`. **`bin/README.md`** indexes the scripts by concern. The two `bin/` Python suites (`test_build_suppliers.py`, `test_build_ev_db.py`) run together via **`make test-scripts`**, which `test.yml` runs on every push/PR before the Kotlin tests.
...
- **`PriceFetcher`** — Interface with a single `fetchPrices(from, to, timeZoneId)` method returning `FetchResult` (prices + source name). `FetchResult` pairs a `List<PriceSlot>` with the data source name (e.g. "ENTSO-E", "EnergyZero"). Decouples `PriceRepository` from a specific API provider. Also defines `sharedHttpClient`, a single `OkHttpClient` (10s connect + 20s read timeout) shared by all API implementations. The read timeout is deliberately loose: ENTSO-E day-ahead responses range from under 1s to ~9s, so a 10s budget converted ordinary latency into `TIMEOUT` failures. The resulting per-source worst case is bounded for the chain as a whole by `FallbackPriceFetcher`'s time budget, so a loose per-request timeout does not multiply by the number of sources in a zone.
...
- **`FallbackPriceFetcher`** — `PriceFetcher` that tries a list of fetchers in order and returns the first result that **actually has prices**: a successful-but-empty response (HTTP 200 with no day-ahead data) falls through to the next source. If no fetcher yields prices it returns an empty result (treated as "not enough data") when at least one responded; the last exception is thrown only when every fetcher failed outright. Used for all multi-source zones (e.g. NL: ENTSO-E → Energy-Charts → EnergyZero). Carries a **chain-level time budget** (`budgetMillis`, default `DEFAULT_BUDGET_MILLIS` = 30s, with an injectable `nanoTime` for tests): because the fetchers run in sequence their per-request timeouts add up, so once the budget is spent the remaining sources are not started. Checked *after each attempt*, not before one and not enforced on the fetchers themselves — an in-flight request is not interrupted, so the total is bounded at roughly the budget plus one source's timeout rather than one timeout per source in the zone. Placing the check after the attempt is what makes the first source unconditional (the budget caps compounding waits, not fetching at all), which in turn guarantees there is an outcome to return or throw; a check placed before the attempt would need an explicit exemption for the first source to avoid ending the chain having run nothing. A fast refusal (an ENTSO-E 503 lands in under ~4s) never trips it, so the budget only bites on a hanging source.
...
- **Energy-Charts** (European fallback) — 30 zones (AT, BE, BG, CH, CZ, DE-LU, DK1, DK2, ES, FR, GR, HR, HU, all six Italian zones, ME, NL, NO2, PL, PT, RO, RS, SE4, SI, SK), 15-min or 60-min resolution, prices in EUR/MWh. Endpoint: `https://api.energy-charts.info/price?bzn={bzn}&start={ISO}&end={ISO}`. No auth required. CC BY 4.0 licensed. Used as fallback when other sources fail.
...
- **Endpoint** — `server/stats/stats.php`, deployed to `aurora:/var/www/stats.sweetspot.today/` via `make deploy-stats` (`bin/deploy/deploy-stats.sh`, scp + the `clear-rate-limit.sh` helper). Setup runbook: `server/stats/README.md`. (The unrelated `server/feedback-worker/` is the Help-section report/feedback Cloudflare Worker.) Behind Cloudflare; the Apache vhost (`stats.sweetspot.today.conf`) maps `POST /report`, `GET /health`, and `GET /rejects` → `stats.php` and denies every other path (so a dashboard pinging `/` logs harmless 403s). It validates the JSON payload, rate-limits per IP (5 min), converts to line protocol, and writes to **InfluxDB 3 Core** (`db=sweetspot`, measurement `api_fetch`) on `localhost:8181`. **The vhost carries the route map, so it and `stats.php` must deploy together** — a new route in the script is denied by the live vhost until that is installed too; `deploy-stats.sh` diffs it and installs it (prompting for sudo) rather than leaving it hand-copied. `INFLUX_URL` overrides the InfluxDB endpoint, which lets the script be smoke-tested against a stub; it is unset in production.
- **`GET /health`** — liveness probe for the uptime monitor. Writes one point to `ingest_canary` and returns `200 {"ok":true,"probe":"ingest"}` only once InfluxDB acks it, else `503`, so it proves the whole path (PHP → token → InfluxDB write) rather than just Apache being up. Answered **before** the POST/User-Agent checks (a monitor sends a plain GET with its own UA) and **before** the rate limiter (which would 429 anything polling faster than once per 5 min). Its canary lives in its own measurement so it can never skew the reliability figures and can be dropped wholesale — InfluxDB 3 Core has no row `DELETE`, but whole tables can be dropped.
...
- **Monitoring — three signals, because "no data arrived" has three different causes.** A single success-only heartbeat conflates them: a quiet weekend and a broken pipeline are indistinguishable from its silence. The endpoint writes `api_fetch` (app reports), `ingest_canary` (health probes), and `endpoint_reject` (every non-2xx):
  1. **Is the path working?** — a Kuma **keyword** monitor polls `GET /health` every 5 min (matching `"probe":"ingest"`, *not* `"ok"` — the failure body contains `"ok":false`). Monitor-driven, so it stays meaningful through quiet periods, and detects breakage in ~10 min instead of ~3 days.
  2. **Are reports flowing?** — the original **push** monitor (id 122, 3-day heartbeat), pinged by `ping_kuma()` after every successful write. Traffic-coupled *by design*; it is named "…reports flowing (traffic-driven)" so that when it fires during a genuine lull the meaning is unambiguous rather than mistaken for an outage. It keeps its notification.
  3. **Are reports being rejected?** — `error_response()` records every non-2xx to `endpoint_reject` (tags: `code`, `reason`, `synthetic`); `GET /rejects` counts the last 6 h with `synthetic='false' AND code != '429'` and a Kuma **keyword** monitor watches for `"clean":true` (15 min). This is the mode a success-only heartbeat cannot see and the one that actually bit us: the Apr–Jun 2026 `status`-whitelist outage returned 400s for three months. Note that signal 1 is *blind* to it — a canary with a hardcoded-valid payload sails through a validation rule the app has outgrown, which is exactly why both exist. The alert clears itself as the window moves past the last rejection. Kuma truncates a body to **47 chars** in its failure message, so `clean` and `rejects` lead the JSON to keep the count visible; the Grafana link (dashboard `sweetspot-api-stats`, `panel-12` "Endpoint rejections") lives in the monitor's `description` instead, and reaches Telegram only if a notification **template** (`{{ monitorJSON.description }}`, Liquid) is enabled. A template applies to every monitor sharing the notification, so guard optional lines with `{% if %}`; never interpolate `{{ monitorJSON }}` wholesale, as it is serialised with `includeSensitiveData = true` (`basic_auth_pass`, `bearer_token`, `pushToken`, `oauth_client_secret`, …).
  Tags in `endpoint_reject` carry only the HTTP code and `stats.php`'s own reason literal, never request data, so a crafted payload cannot inject line protocol.
- **Managing Kuma** — no REST API exists; use the socket.io CLI: `docker exec kuma-api python /app/kuma_client.py <list|get|add|clone|edit|...>` (lives at `aurora:/opt/uptime-kuma`). `add` takes **one whole JSON monitor object** and `edit <id> <json>` patches fields — there are no per-field flags. Copy the shape from an existing monitor with `get <id>` (e.g. `parent: 135` for the `Infrastructure / SweetSpot` group, `notificationIDList: {"1": true}` for the shared channel). Creating a push monitor via the API does **not** auto-generate a `pushToken` — set it explicitly with `edit`.
...
- **Deliberate rejections must carry `X-SweetSpot-Synthetic: 1`** — the same idea one layer up, for `endpoint_reject`. The header tags the row `synthetic='true'` so the rejection alert ignores it; `test.sh` sends it on every request, since a single run of the suite provokes a dozen 4xx on purpose and would otherwise look exactly like the endpoint turning away real reports. Spoofing it only suppresses the spoofer's own alerts, so it is not a security boundary.
