...
- **Per-module gates** — after the coverage passes, each module carries its own Kover verify rule (line coverage) in its `build.gradle.kts`: **`:shared` ≥98, `:app` ≥97, `:wear` ≥93**. CI runs `./gradlew koverVerifyDebug --continue` as the final step (after the uploads, so coverage/test data is still captured when a gate fails; `--continue` reports every failing module, not just the first). Chosen over a Codecov status target because this repo pushes straight to `main` — a Codecov status has nothing to block, whereas the Kover rule fails the build itself, locally and in CI. Each bound sits ~2 points under its actual number — tight enough to catch a real regression while tolerating the defensive/DI/edge lines that survive the presentation exclusions.
