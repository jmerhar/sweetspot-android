...
> **Scope:** the Kover setup, the exclusions and the reasoning for them below describe this repo. The
> summarising, gating and publishing are not implemented here — that logic is shared by every project
> on the coverage site, lives in `jmerhar/coverage`, is invoked from `test.yml` as composite actions,
> and is configured by `coverage.toml` in this repo's root.
...
- **Per-module gates (XML-based, not `koverVerifyDebug`)** — after the coverage passes, CI enforces a per-module line-coverage gate: **`:shared` ≥98, `:app` ≥97, `:wear` ≥93** (thresholds in `bin/quality/coverage-report.py`'s `GATES`). The final CI step runs `python3 bin/quality/coverage-report.py --gate`, which parses the Kover XML reports (generated earlier in the job) and exits non-zero on a regression — after the uploads, so coverage/test data is still captured when a gate fails. Chosen over a Codecov status target because this repo pushes straight to `main` (a Codecov status has nothing to block), and over `./gradlew koverVerifyDebug` because **Kover 0.9.8's verification task does not reliably apply wildcard `classes(...)` excludes** (e.g. `today.sweetspot.ui.*`) — it measured `:app` at ~67% (counting all the Compose UI) where the XML report correctly shows ~99%, and the behaviour even differed between JDK 23 (local) and JDK 17 (CI). The XML reports apply the excludes correctly and consistently, so the gate reads those. (For the same wildcard reason, `:app`'s `excludes` enumerates each `ui` sub-package explicitly rather than trusting one `ui.*` to cross package dots.) Each bound sits ~2 points under its actual number — tight enough to catch a real regression while tolerating the defensive/DI/edge lines that survive the presentation exclusions.
- **README badge = Codecov total** — the badge shows the combined coverage across all three flags (`graph/badge.svg`, no `?flag=`), not just `:shared`. Per-flag numbers remain browsable on Codecov.
