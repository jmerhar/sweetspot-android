---
title: "Muutosloki"
description: "SweetSpotin versiohistoria ja julkaisutiedot."
---

{{< changelog version="5.3" date="5. huhtikuuta 2026" >}}
- Vuositilaus korvaa kertaostoksen — 14 päivän ilmainen kokeilu pysyy ennallaan
- Sovellus tarkistaa tilauksen tilan uudelleen palatessaan etualalle
- Käännösparannuksia unkarissa, romaniassa, puolassa, bulgariassa ja montenegrossa
{{< /changelog >}}

{{< changelog version="5.2" date="2. huhtikuuta 2026" >}}
- Uudistetut laitekuvakkeet — 30 korkealaatuista Material Symbols -kuvaketta paremmilla vastaavuuksilla ja uusilla kuvakkeilla vedenkeittimiä, porealtaita, sprinklereitä ja muuta varten
- Kuvakevalitsin näyttää nyt valitun kuvakkeen nimen käännettynä kaikille 25 tuetulle kielelle
{{< /changelog >}}

{{< changelog version="5.1.5" date="1. huhtikuuta 2026" >}}
- Natiivit virheenkorjaussymbolit sisällytetty julkaisupaketteihin parempaa Play Storen kaatumisraportointia varten
{{< /changelog >}}

{{< changelog version="5.1.4" date="1. huhtikuuta 2026" >}}
- Kiitosvahvistus SweetSpotin tilaamisen jälkeen
- Korjattu vanhan kielen välähdys sovelluksen kielen vaihdon yhteydessä asetuksissa
{{< /changelog >}}

{{< changelog version="5.1.3" date="1. huhtikuuta 2026" >}}
- Asetusten uudelleenjärjestely: tietolähteet, välimuisti ja kehittäjäasetukset ovat nyt Lisäasetukset-osiossa
- Maaluettelo lajitellaan oikein kaikilla kielillä, myös diakriittisten merkkien osalta
- Luonnollisuuden ja kieliopin parannuksia useilla kielillä
{{< /changelog >}}

{{< changelog version="5.1.2" date="30. maaliskuuta 2026" >}}
- Lisätty aikainen tilausvaihtoehto asetuksissa kokeilujakson aikana
{{< /changelog >}}

{{< changelog version="5.1.1" date="30. maaliskuuta 2026" >}}
- Korjattu puhelimen ja kellon erilliset versiokoodit Play Consolen latausta varten
{{< /changelog >}}

{{< changelog version="5.0" date="30. maaliskuuta 2026" >}}
- 14 päivän ilmainen kokeilu vuositilauksella sovelluksen käytön jatkamiseksi
- Maksumuurinäkymä kokeilujakson jälkeen tilauksen palautusmahdollisuudella
- Kokeilujakson laskuri päänäkymässä
- Wear OS -kello näyttää viestin tilaamisesta puhelimella, kun kokeilujakso päättyy
- Korjattu päällekkäisten ENTSO-E TimeSeries -tietueiden deduplikaatio
- Sovelluksen versio näkyy asetusnäkymän alareunassa
{{< /changelog >}}

{{< changelog version="4.1" date="30. maaliskuuta 2026" >}}
- Valinnainen nimetön API-luotettavuustilastointi tietolähteiden laadun parantamiseksi
- Parannettu virheenkäsittely kaikissa viidessä tietolähteessä
{{< /changelog >}}

{{< changelog version="4.0" date="28. maaliskuuta 2026" >}}
- Uusi sovellustunnus: `today.sweetspot`
- Verkkosivuston parannukset ja sivuston validointi
{{< /changelog >}}

{{< changelog version="3.5" date="28. maaliskuuta 2026" >}}
- Lisätty aWATTar varatietolähteeksi Itävallalle ja Saksalle
- Kielikohtainen valuuttamuotoilu EUR-hinnoille
- Käännöslaadun parannuksia 25 kielellä
- Korjattu tulosnäkymän täydellinen päivittyminen 60 sekunnin välein
- Riippuvuudet päivitetty uusimpiin vakaisiin versioihin
{{< /changelog >}}

{{< changelog version="3.4" date="26. maaliskuuta 2026" >}}
- Lisätty Energy-Charts varatietolähteeksi 15 eurooppalaiselle alueelle
- Hintavälimuistin tyhjennys asetuksista ja päivityspainike tulosnäkymässä
- Tarjousalue näkyy nyt tulosnäkymässä
- Parannettu kielioppi käännetyissä teksteissä, jotka sisältävät lukuja
{{< /changelog >}}

{{< changelog version="3.3" date="26. maaliskuuta 2026" >}}
- Asetuksissa näkyy nyt järjestelmäkielesi nimi
- 25 kieltä nyt tuettuna
{{< /changelog >}}

{{< changelog version="3.2" date="5. maaliskuuta 2026" >}}
- Lisätty 21 eurooppalaista kieltä mukaan lukien hollanti, saksa ja ranska (yhteensä 25)
- Koko näytön kielenvalitsin
- Mukautettavat tietolähdeasetukset vetämällä järjesteltäväksi
- Asetusnäkymän uudelleenjärjestely
{{< /changelog >}}

{{< changelog version="3.1" date="4. maaliskuuta 2026" >}}
- ENTSO-E on nyt ensisijainen lähde Alankomaille (EnergyZero varalähteenä)
- Tuntia tarkemman (15 minuutin) hintatarkkuuden tuki
{{< /changelog >}}

{{< changelog version="3.0" date="3. maaliskuuta 2026" >}}
- ENTSO-E Transparency Platformin API-integraatio kaikille eurooppalaisille alueille
- Usean alueen tuki: 30 maata, 43 tarjousaluetta
- Maan ja alueen valinta automaattisella tunnistuksella
- Hinnat tallennetaan paikallisesti nopeampaa latausta varten
{{< /changelog >}}

{{< changelog version="2.3" date="3. maaliskuuta 2026" >}}
- Lisensoitu GPL v3 -lisenssillä
- Ennakoiva takaisin-ele Android 13+:ssa
- Esteettömyysparannuksia (ruudunlukijatuki)
- Vanhentuneen välimuistin käyttö varana, kun uudelleenhaku epäonnistuu
- Vakauden parannuksia ja virhekorjauksia
{{< /changelog >}}

{{< changelog version="2.2" date="2. maaliskuuta 2026" >}}
- Pienempi sovelluskoko
- Turvallisuus- ja vakausparannuksia
- Virhekorjauksia
{{< /changelog >}}

{{< changelog version="2.1" date="2. maaliskuuta 2026" >}}
- Wear OS APK mukana julkaisuissa
- Parannettu suhteellisen ajan näyttö (pyöristetty lähimpään minuuttiin)
{{< /changelog >}}

{{< changelog version="2.0" date="2. maaliskuuta 2026" >}}
- Wear OS -kumppanisovellus automaattisella synkronoinnilla
- Tarkista hinnat ranteestasi tallennetuilla laitteilla
{{< /changelog >}}

{{< changelog version="1.2" date="2. maaliskuuta 2026" >}}
- Korjattu ajoitusongelma, kun edullisin jakso alkaa heti
- Lisätty spot-hinnan vastuuvapauslauseke
{{< /changelog >}}

{{< changelog version="1.1" date="2. maaliskuuta 2026" >}}
- Parannettu käyttöliittymän teksti ja asetusnäkymän asettelu
- Hienosäädetty sovelluskuvake
{{< /changelog >}}

{{< changelog version="1.0" date="2. maaliskuuta 2026" >}}
- Ensimmäinen julkaisu
- Rullaava keston valitsin pikavalintapainikkeilla (1–6 tuntia)
- Mukautettavat laitteet omilla nimillä, kuvakkeilla ja kestoilla
- Tulosnäkymä kustannuserittelyllä aikajaksoittain
- Palkkikaavio tulevista hinnoista edullisimman jakson korostuksella
{{< /changelog >}}
