---
title: "Pakeitimų žurnalas"
description: "SweetSpot versijų istorija ir leidimų pastabos."
---

{{< changelog version="5.3.1" date="2026 m. balandžio 5 d." >}}
- Ištaisytas paleidimo metu įvykstantis gedimas, kurį sukėlė Play Billing Library suderinamumo problema
{{< /changelog >}}

{{< changelog version="5.3" date="2026 m. balandžio 5 d." >}}
- Metinė prenumerata pakeičia vienkartinį pirkimą — 14 dienų nemokamas bandomasis laikotarpis lieka nepakitęs
- Programėlė iš naujo patikrina prenumeratos būseną grįžus į pirmą planą
- Patobulinti vertimai vengrų, rumunų, lenkų, bulgarų ir juodkalniečių kalbomis
{{< /changelog >}}

{{< changelog version="5.2" date="2026 m. balandžio 2 d." >}}
- Atnaujintos prietaisų piktogramos — 30 aukštos kokybės Material Symbols piktogramų su geresniais atitikmenimis ir naujomis piktogramomis virdulių, sūkurinių vonių, laistymo įrenginių ir kt.
- Piktogramų parinkiklis dabar rodo pasirinktos piktogramos pavadinimą, išverstą į visas 25 palaikomas kalbas
{{< /changelog >}}

{{< changelog version="5.1.5" date="2026 m. balandžio 1 d." >}}
- Į leidimo paketus įtraukti vietiniai derinimo simboliai geresniam strigčių ataskaitų teikimui Play Store
{{< /changelog >}}

{{< changelog version="5.1.4" date="2026 m. balandžio 1 d." >}}
- Padėkos patvirtinimas atrakinant SweetSpot
- Ištaisytas senos kalbos blyksnis keičiant programėlės kalbą nustatymuose
{{< /changelog >}}

{{< changelog version="5.1.3" date="2026 m. balandžio 1 d." >}}
- Pertvarkyti nustatymai: duomenų šaltiniai, podėlis ir kūrėjo parinktys dabar skyriuje Išplėstiniai
- Šalių sąrašas teisingai rikiuojamas visomis kalbomis, įskaitant kirčiuotus simbolius
- Pagerintas natūralumas ir gramatika keliomis kalbomis
{{< /changelog >}}

{{< changelog version="5.1.2" date="2026 m. kovo 30 d." >}}
- Pridėta galimybė ankstyvo atrakinimo nustatymuose bandomojo laikotarpio metu
{{< /changelog >}}

{{< changelog version="5.1.1" date="2026 m. kovo 30 d." >}}
- Ištaisyti telefono ir laikrodžio versijų kodai Play Console įkėlimui
{{< /changelog >}}

{{< changelog version="5.0" date="2026 m. kovo 30 d." >}}
- 14 dienų nemokamas bandomasis laikotarpis su vienkartiniu pirkiniu nuolatiniam atrakinimui
- Mokėjimo ekranas pasibaigus bandomajam laikotarpiui su galimybe atkurti ankstesnius pirkinius
- Bandomojo laikotarpio atgalinė atskaita pagrindiniame ekrane
- Wear OS laikrodis rodo pranešimą atrakinti iš telefono, kai bandomasis laikotarpis baigiasi
- Ištaisyta persidengusių ENTSO-E TimeSeries deduplikacija
- Programėlės versija rodoma nustatymų ekrano apačioje
{{< /changelog >}}

{{< changelog version="4.1" date="2026 m. kovo 30 d." >}}
- Neprivaloma anoniminė API patikimumo statistika duomenų šaltinių kokybei gerinti
- Pagerinta klaidų apdorojimas visuose penkiuose duomenų šaltiniuose
{{< /changelog >}}

{{< changelog version="4.0" date="2026 m. kovo 28 d." >}}
- Naujas programėlės ID: `today.sweetspot`
- Svetainės patobulinimai ir validacija
{{< /changelog >}}

{{< changelog version="3.5" date="2026 m. kovo 28 d." >}}
- Pridėtas aWATTar kaip atsarginis duomenų šaltinis Austrijai ir Vokietijai
- Lokalei pritaikytas EUR kainų formatavimas
- Pagerinta vertimų kokybė 25 kalbomis
- Ištaisytas rezultatų ekranas, kad visiškai atsinaujintų kas 60 sekundžių
- Priklausomybės atnaujintos iki naujausių stabilių versijų
{{< /changelog >}}

{{< changelog version="3.4" date="2026 m. kovo 26 d." >}}
- Pridėtas Energy-Charts kaip atsarginis duomenų šaltinis 15 Europos zonų
- Kainų podėlio valymas iš nustatymų ir atnaujinimo mygtukas rezultatų ekrane
- Kainų zona dabar rodoma rezultatų ekrane
- Pagerinta gramatika verčiamuose tekstuose su skaičiais
{{< /changelog >}}

{{< changelog version="3.3" date="2026 m. kovo 26 d." >}}
- Nustatymuose dabar rodomas sistemos kalbos pavadinimas
- Dabar palaikomos 25 kalbos
{{< /changelog >}}

{{< changelog version="3.2" date="2026 m. kovo 5 d." >}}
- Pridėta 21 Europos kalba, įskaitant olandų, vokiečių ir prancūzų (iš viso 25)
- Viso ekrano kalbos parinkiklis
- Konfigūruojamos duomenų šaltinių nuostatos su pertvarkymu vilkimu
- Pertvarkytas nustatymų ekrano išdėstymas
{{< /changelog >}}

{{< changelog version="3.1" date="2026 m. kovo 4 d." >}}
- ENTSO-E dabar yra pagrindinis šaltinis Nyderlandams (EnergyZero kaip atsarginis)
- Kainų raiška mažesnė nei valanda (15 minučių)
{{< /changelog >}}

{{< changelog version="3.0" date="2026 m. kovo 3 d." >}}
- ENTSO-E Transparency Platform API integracija visoms Europos zonoms
- Kelių zonų palaikymas: 30 šalių, 43 prekybos zonos
- Šalies ir zonos pasirinkimas su automatiniu aptikimu
- Kainos saugomos vietiniame įrenginyje greitesniam įkėlimui
{{< /changelog >}}

{{< changelog version="2.3" date="2026 m. kovo 3 d." >}}
- Licencijuota pagal GPL v3
- Nuspėjamasis grįžimo gestas Android 13+
- Prieinamumo patobulinimai (ekrano skaitytojo palaikymas)
- Pasenusio podėlio naudojimas, kai pakartotinis gavimas nepavyksta
- Stabilumo patobulinimai ir klaidų taisymai
{{< /changelog >}}

{{< changelog version="2.2" date="2026 m. kovo 2 d." >}}
- Mažesnis programėlės dydis
- Saugumo ir stabilumo patobulinimai
- Klaidų taisymai
{{< /changelog >}}

{{< changelog version="2.1" date="2026 m. kovo 2 d." >}}
- Wear OS APK įtrauktas į leidimus
- Pagerintas santykinio laiko rodymas (suapvalintas iki artimiausios minutės)
{{< /changelog >}}

{{< changelog version="2.0" date="2026 m. kovo 2 d." >}}
- Wear OS papildoma programėlė su automatiniu sinchronizavimu
- Tikrinkite kainas nuo riešo su išsaugotais prietaisais
{{< /changelog >}}

{{< changelog version="1.2" date="2026 m. kovo 2 d." >}}
- Ištaisyta laiko problema, kai pigiausias langas prasideda iš karto
- Pridėta spot kainų pastaba
{{< /changelog >}}

{{< changelog version="1.1" date="2026 m. kovo 2 d." >}}
- Patobulinti sąsajos tekstai ir nustatymų ekrano išdėstymas
- Patobulinta programėlės piktograma
{{< /changelog >}}

{{< changelog version="1.0" date="2026 m. kovo 2 d." >}}
- Pradinis leidimas
- Slenkamo rato trukmės parinkiklis su greito pasirinkimo mygtukais (1–6 valandos)
- Konfigūruojami prietaisai su pasirinktais pavadinimais, piktogramomis ir trukmėmis
- Rezultatų ekranas su išlaidų pasiskirstymu pagal laiko intervalus
- Stulpelinė diagrama su artėjančiomis kainomis ir paryškintu pigiausiu langu
{{< /changelog >}}
