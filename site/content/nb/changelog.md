---
title: "Endringslogg"
description: "SweetSpot versjonshistorikk og utgivelsesnotater."
---

{{< changelog version="7.0" date="31. juli 2026" >}}
- Hjelp & støtte er nå innebygd: rapporter et problem eller send tilbakemelding fra appen, følg svarene i en samtale og hold oversikt over rapportene dine — uten konto.
- «Ferdig innen» er nå et smart standardvalg: det billigste tidspunktet som fortsatt blir ferdig i tide — bruk «Tidligere» eller «Billigere» for å justere rundt det.
- En kort introduksjon ved første oppstart og engangstips for de mindre åpenbare funksjonene.
- Mer pålitelige priser og klarere ordlyd på alle 25 språk.
{{< /changelog >}}

{{< changelog version="6.6" date="21. juli 2026" >}}
- Totalpris-diagrammet: hver stolpe deles nå i spotpris, energiavgift og leverandørpåslag, med en fargeforklaring.
- Bytt raskt resultatene mellom totalprisen og ren spotpris.
- Trykk og hold på diagrammet for å se den nøyaktige prisen i ethvert øyeblikk (full oppdeling når totalprisen er på).
- Elbillading viser nå den totale ladetiden øverst i resultatene.
{{< /changelog >}}

{{< changelog version="6.5" date="15. juli 2026" >}}
- Grupper apparatknappene etter type — som rader eller kolonner — slik at programmer for samme apparat holdes samlet. Kjøretøy kan ha sin egen seksjon over eller under.
{{< /changelog >}}

{{< changelog version="6.4" date="14. juli 2026" >}}
- Del oppsettet ditt: kopier apparatene dine, rekkefølgen deres og innstillingene for elbillading til en annen telefon ved å skanne en QR-kode eller sende en lenke — ingen konto, fungerer frakoblet. Den andre telefonen viser en forhåndsvisning der du kan legge til, erstatte eller velge hva du vil importere.
{{< /changelog >}}

{{< changelog version="6.3" date="13. juli 2026" >}}
- Sorter og omorganiser apparatene dine: dra dem i din egen rekkefølge, eller sorter etter mest brukt, nylig brukt, navn, varighet eller type.
- «Mest brukt» og «Nylig brukt» teller trykk på både telefonen og klokken.
- Velg hvor kjøretøy vises blant apparatene: blandet inn, først, sist eller egen seksjon.
- Kjøretøy viser nå et bilikon, og datakilder kan også omorganiseres ved å dra.
{{< /changelog >}}

{{< changelog version="6.2" date="12. juli 2026" >}}
- Fornyede innstillinger: en ryddig meny med kategorier, hver med et eget ikon og skjermbilde — Apparater, Elbillading, Totalpris, Region (land, prisområde, tidssone), Utseende (språk, tema) og Avansert. Alt er lettere å finne enn i den forrige lange listen, og når du kommer tilbake fra en velger, beholder du plassen din.
{{< /changelog >}}

{{< changelog version="6.1.1" date="12. juli 2026" >}}
- Totalpris forbedret: påslagsfeltet viser nå valutaen din (f.eks. €/kWh), innstillingene beholder rulleposisjonen når du kommer tilbake fra en velger, og hvis totalprisen er slått på, minner appen deg om å velge en leverandør eller angi et påslag før du forlater innstillingene.
{{< /changelog >}}

{{< changelog version="6.1" date="12. juli 2026" >}}
- Totalpris (Nederland): vis om ønskelig hele forbrukerprisen — spotprisen pluss energiavgift, leverandørpåslag og MVA — i stedet for bare spotprisen. Velg leverandøren din eller angi ditt eget påslag under «Totalpris» i innstillingene. Det endrer aldri hvilken periode som er billigst; det viser bare en mer realistisk kostnad.
{{< /changelog >}}

{{< changelog version="6.0.1" date="25. juni 2026" >}}
- Forbedrede og korrigerte oversettelser på alle støttede språk for klarere og mer naturlig ordlyd.
{{< /changelog >}}

{{< changelog version="6.0" date="23. juni 2026" >}}
- Elbillading: legg til bilen din, oppgi nåværende og ønsket ladenivå, så finner SweetSpot det billigste tidspunktet å koble til og hvor lang tid det tar.
- «Ferdig innen»-tidspunkt: få et hvilket som helst apparat eller en lading ferdig innen en frist du selv setter.
- Effekt: angi et apparats effekt i kW slik at kostnadsestimatet gjenspeiler det reelle forbruket.
{{< /changelog >}}

{{< changelog version="5.4" date="22. juni 2026" >}}
- Nye knapper «Tidligere» og «Billigere» lar deg velge et tidligere tidspunkt når den billigste perioden ikke passer, og viser hvor mye mer det koster.
{{< /changelog >}}

{{< changelog version="5.3.2" date="9. april 2026" >}}
- Oppdaterte avhengigheter for forbedret stabilitet
{{< /changelog >}}

{{< changelog version="5.3.1" date="5. april 2026" >}}
- Rettet krasj ved oppstart forårsaket av et kompatibilitetsproblem med Play Billing Library
{{< /changelog >}}

{{< changelog version="5.3" date="5. april 2026" >}}
- Årsabonnement erstatter engangskjøpet — den 14 dager lange gratis prøveperioden forblir uendret
- Appen kontrollerer abonnementsstatus på nytt når den kommer tilbake i forgrunnen
- Forbedrede oversettelser på ungarsk, rumensk, polsk, bulgarsk og montenegrinsk
{{< /changelog >}}

{{< changelog version="5.2" date="2. april 2026" >}}
- Fornyede apparatikoner — 30 høykvalitets Material Symbols-ikoner med bedre treff og nye ikoner for vannkokere, boblebad, sprinklere og mer
- Ikonvelgeren viser nå navnet på det valgte ikonet, oversatt til alle 25 støttede språk
{{< /changelog >}}

{{< changelog version="5.1.5" date="1. april 2026" >}}
- Inkludert native feilsøkingssymboler i utgivelsespakker for bedre krasjrapportering i Play Store
{{< /changelog >}}

{{< changelog version="5.1.4" date="1. april 2026" >}}
- Takkebekreftelse etter opplåsing av SweetSpot
- Rettet et glimt av det gamle språket ved endring av appspråket i innstillinger
{{< /changelog >}}

{{< changelog version="5.1.3" date="1. april 2026" >}}
- Omorganiserte innstillinger: datakilder, hurtiglager og utvikleralternativer er nå i en avansert-seksjon
- Landlisten sorteres korrekt for alle språk, inkludert tegn med aksenter
- Forbedret naturlighet og grammatikk på tvers av flere språk
{{< /changelog >}}

{{< changelog version="5.1.2" date="30. mars 2026" >}}
- Lagt til mulighet for tidlig opplåsing i innstillingene i prøveperioden
{{< /changelog >}}

{{< changelog version="5.1.1" date="30. mars 2026" >}}
- Fikset telefon- og klokkebygg slik at de har distinkte versjonskoder for opplasting til Play Console
{{< /changelog >}}

{{< changelog version="5.0" date="30. mars 2026" >}}
- 14 dagers gratis prøveperiode med engangskjøp for permanent opplåsing
- Betalingsvegg etter at prøveperioden utløper, med mulighet for å gjenopprette tidligere kjøp
- Nedtelling for prøveperioden vises på hovedskjermen
- Wear OS-klokken viser en melding om å låse opp fra telefonen når prøveperioden utløper
- Fikset deduplisering av overlappende ENTSO-E TimeSeries
- Appversjon vises nederst på innstillingsskjermen
{{< /changelog >}}

{{< changelog version="4.1" date="30. mars 2026" >}}
- Valgfri anonym API-pålitelighetsstatistikk for å forbedre datakildekvaliteten
- Forbedret feilhåndtering på tvers av alle fem datakilder
{{< /changelog >}}

{{< changelog version="4.0" date="28. mars 2026" >}}
- Ny applikasjons-ID: `today.sweetspot`
- Nettsideforbedringer og sidevalidering
{{< /changelog >}}

{{< changelog version="3.5" date="28. mars 2026" >}}
- Lagt til aWATTar som reservedatakilde for Østerrike og Tyskland
- Språktilpasset valutaformatering for EUR-priser
- Forbedret oversettelseskvalitet på tvers av 25 språk
- Fikset resultatskjermen slik at den oppdateres fullstendig hvert 60. sekund
- Oppdaterte avhengigheter til nyeste stabile versjoner
{{< /changelog >}}

{{< changelog version="3.4" date="26. mars 2026" >}}
- Lagt til Energy-Charts som reservedatakilde for 15 europeiske soner
- Tøm hurtiglager for priser fra innstillingene og oppdateringsknapp på resultatskjermen
- Prissone vises nå på resultatskjermen
- Forbedret grammatikk for oversatt tekst med tall
{{< /changelog >}}

{{< changelog version="3.3" date="26. mars 2026" >}}
- Innstillinger viser nå navnet på systemspråket ditt
- 25 språk støttes nå
{{< /changelog >}}

{{< changelog version="3.2" date="5. mars 2026" >}}
- Lagt til 21 europeiske språk inkludert nederlandsk, tysk og fransk (25 totalt)
- Fullskjerms språkvelger
- Konfigurerbare datakildeprioriteter med dra-for-å-omorganisere
- Omorganisert oppsett for innstillingsskjermen
{{< /changelog >}}

{{< changelog version="3.1" date="4. mars 2026" >}}
- ENTSO-E er nå hovedkilden for Nederland (EnergyZero som reserve)
- Støtte for 15 minutters prisoppløsning
{{< /changelog >}}

{{< changelog version="3.0" date="3. mars 2026" >}}
- ENTSO-E Transparency Platform API-integrasjon for alle europeiske soner
- Flersonestøtte: 30 land, 43 budområder
- Land- og sonevalg med automatisk gjenkjenning
- Priser lagres lokalt for raskere lasting
{{< /changelog >}}

{{< changelog version="2.3" date="3. mars 2026" >}}
- Lisensiert under GPL v3
- Prediktiv tilbake-gest på Android 13+
- Tilgjengelighetsforbedringer (støtte for skjermleser)
- Utdatert hurtiglager som reserve når ny henting mislykkes
- Stabilitets- og feilrettinger
{{< /changelog >}}

{{< changelog version="2.2" date="2. mars 2026" >}}
- Mindre appstørrelse
- Sikkerhets- og stabilitetsforbedringer
- Feilrettinger
{{< /changelog >}}

{{< changelog version="2.1" date="2. mars 2026" >}}
- Wear OS APK inkludert i utgivelser
- Forbedret relativ tidsvisning (avrundet til nærmeste minutt)
{{< /changelog >}}

{{< changelog version="2.0" date="2. mars 2026" >}}
- Wear OS-følgeapp med automatisk synkronisering
- Sjekk priser fra håndleddet med lagrede apparater
{{< /changelog >}}

{{< changelog version="1.2" date="2. mars 2026" >}}
- Fikset et tidsproblem når den billigste perioden starter umiddelbart
- Lagt til ansvarsfraskrivelse for spotpriser
{{< /changelog >}}

{{< changelog version="1.1" date="2. mars 2026" >}}
- Forbedret grensesnittekst og oppsett for innstillingsskjermen
- Forfinet appikon
{{< /changelog >}}

{{< changelog version="1.0" date="2. mars 2026" >}}
- Første utgivelse
- Rullhjul for varighetsvelger med hurtigknapper (1–6 timer)
- Konfigurerbare apparater med egne navn, ikoner og varigheter
- Resultatskjerm med kostnadsfordeling per tidsluke
- Stolpediagram som viser kommende priser med billigste periode uthevet
{{< /changelog >}}
