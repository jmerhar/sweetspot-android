---
title: "Ändringslogg"
description: "SweetSpots versionshistorik och versionskommentarer."
---

{{< changelog version="7.0" date="31 juli 2026" >}}
- Hjälp & support, inbyggt i appen: rapportera ett problem eller skicka feedback direkt i appen, följ svaren i en konversation och håll koll på dina rapporter — inget konto behövs.
- ”Klar senast” är nu ett smart standardval: den billigaste tiden som ändå hinner bli klar i tid — stega Tidigare eller Billigare runt den.
- En snabb introduktion vid första starten och engångstips för de mindre uppenbara funktionerna.
- Mer tillförlitliga priser och tydligare formuleringar på alla 25 språk.
{{< /changelog >}}

{{< changelog version="6.6" date="21 juli 2026" >}}
- Totalprisdiagrammet: varje stapel delas nu upp i spotpris, energiskatt och leverantörens påslag, med en förklaring.
- Växla snabbt resultaten mellan totalpriset och enbart spotpriset.
- Tryck och håll på diagrammet för att se exakt pris vid vilken tidpunkt som helst (fullständig uppdelning när totalpriset är på).
- Elbilsladdning visar nu den totala laddningstiden i resultatrubriken.
{{< /changelog >}}

{{< changelog version="6.5" date="15 juli 2026" >}}
- Gruppera apparatknapparna efter typ – som rader eller kolumner – så att program för samma apparat hålls samlade. Fordon kan ha en egen sektion ovanför eller nedanför.
{{< /changelog >}}

{{< changelog version="6.4" date="14 juli 2026" >}}
- Dela din uppsättning: kopiera dina apparater, deras ordning och inställningar för laddning av elbil till en annan telefon genom att skanna en QR-kod eller skicka en länk — inget konto, fungerar offline. Den andra telefonen visar en förhandsgranskning där du kan lägga till, ersätta eller välja vad som ska importeras.
{{< /changelog >}}

{{< changelog version="6.3" date="13 juli 2026" >}}
- Sortera och ordna om dina apparater: dra dem i egen ordning eller sortera efter Mest använda, Senast använda, Namn, Varaktighet eller Typ.
- ”Mest använda” och ”Senast använda” räknar tryck på både telefon och klocka.
- Välj var fordon visas bland apparaterna: Blandat, Först, Sist eller Egen sektion.
- Fordon visas nu med en bilikon, och datakällor kan också ordnas om genom att dra.
{{< /changelog >}}

{{< changelog version="6.2" date="12 juli 2026" >}}
- Omgjorda inställningar: en prydlig meny med kategorier, var och en med en ikon och en egen skärm — Apparater, Laddning av elbil, Totalpris, Region (land, prisområde, tidszon), Utseende (språk, tema) och Avancerat. Allt är lättare att hitta än i den tidigare enda långa listan, och när du kommer tillbaka från en väljare behåller du din plats.
{{< /changelog >}}

{{< changelog version="6.1.1" date="12 juli 2026" >}}
- Förbättringar av totalpriset: påslagsfältet visar nu din valuta (t.ex. €/kWh), inställningarna behåller din rullningsposition när du kommer tillbaka från en väljare, och om totalpriset är påslaget påminns du om att välja en leverantör eller ange ett påslag innan du lämnar.
{{< /changelog >}}

{{< changelog version="6.1" date="12 juli 2026" >}}
- Totalpris (Nederländerna): visa valfritt hela konsumentpriset — spotpriset plus energiskatt, din leverantörs påslag och moms — i stället för enbart marknadspriset. Välj din leverantör eller ange ett eget påslag under ”Totalpris” i inställningarna. Det ändrar aldrig vilken tidsperiod som är billigast — det visar bara en realistisk körkostnad.
{{< /changelog >}}

{{< changelog version="6.0.1" date="25 juni 2026" >}}
- Förbättrade och korrigerade översättningar på alla språk som stöds för tydligare och mer naturligt språk.
{{< /changelog >}}

{{< changelog version="6.0" date="23 juni 2026" >}}
- Elbilsladdning: lägg till din bil, ange aktuell och önskad laddningsnivå, så hittar SweetSpot den billigaste tiden att ladda och hur lång tid det tar.
- ”Klar senast”-tid: få vilken apparat eller laddning som helst färdig till en tidsgräns du anger.
- Effekt: ange en apparats effekt i kW så att kostnadsberäkningen återspeglar dess verkliga förbrukning.
{{< /changelog >}}

{{< changelog version="5.4" date="22 juni 2026" >}}
- Nya knappar ”Tidigare” och ”Billigare” låter dig välja en tidigare tid när den billigaste tidsperioden inte passar, och visar hur mycket mer det kostar.
{{< /changelog >}}

{{< changelog version="5.3.2" date="9 april 2026" >}}
- Uppdaterade beroenden för förbättrad stabilitet
{{< /changelog >}}

{{< changelog version="5.3.1" date="5 april 2026" >}}
- Åtgärdat krasch vid start orsakad av ett kompatibilitetsproblem med Play Billing Library
{{< /changelog >}}

{{< changelog version="5.3" date="5 april 2026" >}}
- Årsprenumeration ersätter engångsköpet — den 14 dagar långa gratis provperioden förblir oförändrad
- Appen kontrollerar prenumerationsstatus igen när den återgår till förgrunden
- Förbättrade översättningar på ungerska, rumänska, polska, bulgariska och montenegrinska
{{< /changelog >}}

{{< changelog version="5.2" date="2 april 2026" >}}
- Förnyade apparatikoner — 30 högkvalitativa Material Symbols-ikoner med bättre matchningar och nya ikoner för vattenkokare, bubbelpooler, sprinklers och mer
- Ikonväljaren visar nu namnet på den valda ikonen, översatt till alla 25 språk som stöds
{{< /changelog >}}

{{< changelog version="5.1.5" date="1 april 2026" >}}
- Inkluderat inbyggda felsökningssymboler i releasepaket för bättre kraschrapportering i Play Store
{{< /changelog >}}

{{< changelog version="5.1.4" date="1 april 2026" >}}
- Tackbekräftelse efter upplåsning av SweetSpot
- Åtgärdat en blixt av det gamla språket vid ändring av appspråket i inställningar
{{< /changelog >}}

{{< changelog version="5.1.3" date="1 april 2026" >}}
- Omorganiserade inställningar: datakällor, cache och utvecklaralternativ finns nu under Avancerat
- Landlistan sorteras korrekt för alla språk, inklusive tecken med accenter
- Förbättrad naturlighet och grammatik på flera språk
{{< /changelog >}}

{{< changelog version="5.1.2" date="30 mars 2026" >}}
- Lagt till möjlighet till tidig upplåsning i Inställningar under provperioden
{{< /changelog >}}

{{< changelog version="5.1.1" date="30 mars 2026" >}}
- Telefon- och klockbyggen har nu separata versionskoder för uppladdning till Play Console
{{< /changelog >}}

{{< changelog version="5.0" date="30 mars 2026" >}}
- 14 dagars gratis provperiod med engångsköp för permanent upplåsning
- Betalningsskärm efter provperiodens slut med möjlighet att återställa tidigare köp
- Nedräkning av provperioden på huvudskärmen
- Wear OS-klockan visar ett meddelande om att låsa upp från telefonen när provperioden löper ut
- Åtgärdad deduplicering av överlappande ENTSO-E TimeSeries
- Appversion visas längst ner på inställningsskärmen
{{< /changelog >}}

{{< changelog version="4.1" date="30 mars 2026" >}}
- Valfri anonym API-tillförlitlighetsstatistik för att förbättra datakällornas kvalitet
- Förbättrad felhantering för alla fem datakällor
{{< /changelog >}}

{{< changelog version="4.0" date="28 mars 2026" >}}
- Nytt program-ID: `today.sweetspot`
- Förbättringar av webbplatsen och webbplatsvalidering
{{< /changelog >}}

{{< changelog version="3.5" date="28 mars 2026" >}}
- Lagt till aWATTar som reservdatakälla för Österrike och Tyskland
- Regionanpassad valutaformatering för EUR-priser
- Förbättrad översättningskvalitet på 25 språk
- Resultatskärmen uppdateras nu fullständigt var 60:e sekund
- Beroenden uppdaterade till senaste stabila versioner
{{< /changelog >}}

{{< changelog version="3.4" date="26 mars 2026" >}}
- Lagt till Energy-Charts som reservdatakälla för 15 europeiska zoner
- Rensa priscache från inställningar och uppdateringsknapp på resultatskärmen
- Elområdet visas nu på resultatskärmen
- Förbättrad grammatik för översatt text med siffror
{{< /changelog >}}

{{< changelog version="3.3" date="26 mars 2026" >}}
- Inställningarna visar nu systemspråkets namn
- 25 språk stöds nu
{{< /changelog >}}

{{< changelog version="3.2" date="5 mars 2026" >}}
- Lagt till 21 europeiska språk inklusive nederländska, tyska och franska (25 totalt)
- Helskärms språkväljare
- Konfigurerbara datakällepreferenser med dra-för-att-ordna
- Omorganiserad layout för inställningsskärmen
{{< /changelog >}}

{{< changelog version="3.1" date="4 mars 2026" >}}
- ENTSO-E är nu primär källa för Nederländerna (EnergyZero som reserv)
- Stöd för prisupplösning under en timme (15 minuter)
{{< /changelog >}}

{{< changelog version="3.0" date="3 mars 2026" >}}
- Integration med ENTSO-E Transparency Platform API för alla europeiska zoner
- Stöd för flera zoner: 30 länder, 43 elområden
- Val av land och zon med automatisk identifiering
- Priser lagrade lokalt för snabbare laddning
{{< /changelog >}}

{{< changelog version="2.3" date="3 mars 2026" >}}
- Licensierad under GPL v3
- Prediktiv tillbaka-gest på Android 13+
- Förbättrad tillgänglighet (stöd för skärmläsare)
- Användning av gammal cache när uppdatering misslyckas
- Stabilitetsförbättringar och buggfixar
{{< /changelog >}}

{{< changelog version="2.2" date="2 mars 2026" >}}
- Mindre appstorlek
- Säkerhets- och stabilitetsförbättringar
- Buggfixar
{{< /changelog >}}

{{< changelog version="2.1" date="2 mars 2026" >}}
- Wear OS APK inkluderad i utgåvorna
- Förbättrad relativ tidsvisning (avrundad till närmaste minut)
{{< /changelog >}}

{{< changelog version="2.0" date="2 mars 2026" >}}
- Kompletterande Wear OS-app med automatisk synkronisering
- Kolla priser direkt på handleden med sparade apparater
{{< /changelog >}}

{{< changelog version="1.2" date="2 mars 2026" >}}
- Åtgärdat ett tidsproblem när den billigaste tidsperioden börjar omedelbart
- Lagt till ansvarsfriskrivning om spotpriser
{{< /changelog >}}

{{< changelog version="1.1" date="2 mars 2026" >}}
- Förbättrad gränssnittstext och layout för inställningsskärmen
- Förfinad appikon
{{< /changelog >}}

{{< changelog version="1.0" date="2 mars 2026" >}}
- Första utgåvan
- Rullhjulsväljare för körtid med snabbknappar (1–6 timmar)
- Konfigurerbara apparater med egna namn, ikoner och körtider
- Resultatskärm med kostnadsuppdelning per tidslucka
- Stapeldiagram med kommande priser och den billigaste tidsperioden markerad
{{< /changelog >}}
